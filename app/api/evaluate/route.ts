  ...
  try {
    ...
    if (!apiKey || apiKey === 'placeholder') {
      return NextResponse.json({
        "c1": true, "c3": true, "c4": true, "c5": true, "c6": true,
        "c7": content.length >= 3000, "c8": true, "c9": true, "c10": true,
...
아래 블로그 "본문"과 "메인 키워드"를 읽고, 21가지 점검 기준을 충족했는지 (true/false)로만 평가하여 순수한 JSON 객체(Object) 형태로 반환하세요.
...
c7: 글자 수: 공백 포함 3,000자 이상인가? (현재 약 ${content.length}자로 참고하여 판단)
c8: 문단 구조: 적절한 단락 나누기와 리스트형 마크다운(-, 1.)이 원활히 사용되었는가?
c9: 정의 문장: "~란 무엇인가" 형태의 기본 개념 설명이 포함되었는가?
c10: FAQ: 최소 3~4개 이상의 Q&A 항목이 존재하는가?
...
c20: 최신 출처 활용: 2022년 이후의 공신력 있는 출처(논문, 정부자료, 뉴스 등)가 5개 이상 인용되었는가?
c21: 참고 자료 목록: 문서 하단에 "참고 자료" 섹션과 출처 리스트가 명확히 포함되었는가?
